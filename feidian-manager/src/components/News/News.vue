...
<script>
import {update,_delete,adddata,getdata,_search} from '@/networks/news'
export default {
  data () {
    return {
      address: '',
  ...
  methods: {
    ...
    handleDelete (index, row) {
      this.confirmDelete().then(() => {
        //this.axios.post('/news/delete', row)
        _delete(row)
        .then(res => {
          this._getData()
        })
    ...
    submitForm () {
      ...
      if (this.handleAddress) {//handleAddress有值了，所以是修改(update)
        ...
        // })
        //this.axios.post('/news/update', form, config)
        update(form)
        .then(res => {
          this.handleAddress = ''
          this.emptyProperty(this.form, /news/)
        ...
        //   param.append(key.toString(), form[key].toString())
        // })
        //this.axios.post('/news/add', form, config)
        adddata(form)
        .then(res => {
          //console.log(res)
          this.emptyProperty(this.form, /news/)
    ...
    search (title) {
      //this.axios.post('/news/search', { newsTitle: title })
      _search({ newsTitle: title })
      .then(res => {
        this.tableData = []
