<template>
  <div>
    ...
    <div v-if="imgs">
      <div class='img_button_wrapper' v-for="img in imgs" :key="img.whereaboutsId">
        <div class="img_button">

          <img :src="img.imgUrl" width="200" height="100" />
          <div class="delete_button">
            <el-button type="danger" @click="deleteImg(img.whereaboutsId)">刪除图片</el-button>
          </div>
        </div>
      </div>
    </div>
...
<script>
import { _delete,upload,getdata } from '@/networks/whereabouts'
export default {
  data () {
    return {
      imgs: [],
  ...
  methods: {
    ...
    deleteImg (whereaboutsId) {
      let param = {}
      param = new FormData(); //创建form对象
      param.append('whereaboutsId', whereaboutsId);//通过append向form对象添加数据
      _delete(param).then(res => {
